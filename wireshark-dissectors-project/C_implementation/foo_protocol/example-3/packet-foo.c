...
static const int FOO_UDP_PORTS[] = {8080, 5678, 9000};
static const int NUM_FOO_UDP_PORTS = sizeof(FOO_UDP_PORTS)/sizeof(FOO_UDP_PORTS[0]);
...
//To associate traffic on mutiplr UPD port  with foo protocol
void proto_reg_handoff_foo(void)
{
    for (int i = 0; i < NUM_FOO_UDP_PORTS; i++) {
        dissector_add_uint("udp.port", FOO_UDP_PORTS[i], foo_handle);
    }
}